 ...
 *   npx tsx scripts/generate-people.ts [count]
 * 
 * Examples:
...
interface ScoutDataRecord {
  ...
  propValcalc?: string;
  propYrbld?: string;
  propLivingsqft?: string;
  propBedrms?: string;
  propBaths?: string;
  propPool?: string;
  propFrpl?: string;
  cpiInsuranceIndex?: number;
  cpiHealthIndex?: number;
  cpiHomeLivIndex?: number;
  creditcard?: string;
  charitydnr?: string;
  dnc?: string;
}
...
function generatePersonRecord(location: Location, index: number): {
  person: PersonRecord;
  scoutData: ScoutDataRecord;
} {
  ...
  const scoutData: ScoutDataRecord = {
    age,
    ehiV2: income,
    wealthscrV2: wealth,
    ...
    child: Math.random() > 0.6 ? 'Y' : undefined, // 40% have children
    veterancd: Math.random() > 0.9 ? 'Y' : undefined, // 10% veterans
    ...
    propPool: Math.random() > 0.7 ? 'Y' : undefined, // 30% have pools
    propFrpl: Math.random() > 0.5 ? 'Y' : undefined, // 50% have fireplaces
    ...
    creditcard: Math.random() > 0.2 ? 'Y' : undefined, // 80% have credit cards
    charitydnr: Math.random() > 0.7 ? 'Y' : undefined, // 30% charity donors
    dnc: Math.random() > 0.8 ? 'Y' : undefined, // 20% on do not call list
  };
  ...
  for (let i = 0; i < count; i += batchSize) {
    ...
    try {
      for (let k = 0; k < batch.length; k++) {
        const { person, scoutData } = batch[k];
        try {
          // Create person first
          ...
          console.error(`❌ Error creating person ${i + k}:`, error);
          errors++;
        }
      }
