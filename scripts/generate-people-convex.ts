...
function generateBangladeshPhoneNumber(): string {
  const operator = faker.helpers.arrayElement(['017', '018', '019', '015', '016']);
...
// Function to generate Bangladesh house values (in USD)
function generateBangladeshHouseValue(): number {
  // Bangladesh house values are typically much lower
  return faker.number.int({ min: 15000, max: 80000 });
}

// Function to generate scout data for a person
function generateScoutData(personId: string) {
  return {
    personId,
    age: faker.number.int({ min: 25, max: 75 }),
    ehiV2: faker.helpers.arrayElement(['A', 'B', 'C', 'D', 'E']), // income bracket
    wealthscrV2: faker.helpers.arrayElement(['A', 'B', 'C', 'D', 'E']), // wealth bracket
    homeownercd: faker.helpers.arrayElement(['Y', 'N']), // homeowner
    lor: faker.number.int({ min: 1, max: 30 }), // length of residence in years
    educationcd: faker.helpers.arrayElement(['HS', 'COL', 'GRAD', 'PHD']), // education
    marriedcd: faker.helpers.arrayElement(['Y', 'N', 'D', 'W']), // marital status
    child: faker.helpers.arrayElement(['Y', 'N']), // has children
    veterancd: faker.helpers.arrayElement(['Y', 'N']), // veteran
    propValcalc: faker.number.int({ min: 50000, max: 2000000 }).toString(),
    propYrbld: faker.number.int({ min: 1950, max: 2024 }).toString(),
    propLivingsqft: faker.number.int({ min: 800, max: 5000 }).toString(),
    propBedrms: faker.number.int({ min: 1, max: 6 }).toString(),
    propBaths: faker.number.int({ min: 1, max: 4 }).toString(),
    propPool: faker.helpers.arrayElement(['Y', 'N']),
    propFrpl: faker.helpers.arrayElement(['Y', 'N']), // fireplace
    cpiInsuranceIndex: faker.number.float({ min: 0.8, max: 1.2 }),
    cpiHealthIndex: faker.number.float({ min: 0.8, max: 1.2 }),
    cpiHomeLivIndex: faker.number.float({ min: 0.8, max: 1.2 }),
    creditcard: faker.helpers.arrayElement(['Y', 'N']),
    charitydnr: faker.helpers.arrayElement(['Y', 'N']), // charity donor
    dnc: faker.helpers.arrayElement(['Y', 'N']), // do not call
  };
}

async function generatePeopleConvex(count: number) {
  console.log(`🌎 Generating ${count} people for Convex database...`);
  
  const startTime = Date.now();
  let processed = 0;
  const batchSize = 100; // Smaller batch size for Convex
  
  // Calculate distribution: 50% in earthquake-prone zones (high risk), 30% medium risk, 20% low risk
  ...
  console.log(`📊 Distribution: ${highRiskCount} high-risk, ${mediumRiskCount} medium-risk, ${lowRiskCount} low-risk`);
  
  // Filter cities by risk level
  const highRiskCities = usCities.filter(city => city.risk === 'high');
  const mediumRiskCities = usCities.filter(city => city.risk === 'medium');
  const lowRiskCities = usCities.filter(city => city.risk === 'low');
  
  console.log(`🏙️ Cities: ${highRiskCities.length} high-risk, ${mediumRiskCities.length} medium-risk, ${lowRiskCities.length} low-risk`);
  ...
  for (let i = 0; i < highRiskCount; i += batchSize) {
    ...
    for (const person of batch) {
      try {
        await convex.mutation(api.persons.create, person);
        processed++;
      } catch (error) {
  ...
  // Generate medium-risk customers
  console.log('🏠 Generating medium-risk customers...');
  for (let i = 0; i < mediumRiskCount; i += batchSize) {
    const batchSizeActual = Math.min(batchSize, mediumRiskCount - i);
    const batch = [];
    
    for (let j = 0; j < batchSizeActual; j++) {
      const city = faker.helpers.arrayElement(mediumRiskCities);
      const firstName = faker.person.firstName();
      const lastName = faker.person.lastName();
      const email = faker.internet.email({ firstName, lastName });
      ...
      const person = {
        ...
        latitude: city.lat + (faker.number.float({ min: -0.1, max: 0.1 })),
        longitude: city.lng + (faker.number.float({ min: -0.1, max: 0.1 })),
        houseValue: generateHouseValue(city, 'medium'),
    ...
    console.log(`✅ Processed ${processed}/${mediumRiskCount} medium-risk customers`);
  }
  
  // Generate low-risk customers
  console.log('🏡 Generating low-risk customers...');
  for (let i = 0; i < lowRiskCount; i += batchSize) {
    const batchSizeActual = Math.min(batchSize, lowRiskCount - i);
    const batch = [];
    
    for (let j = 0; j < batchSizeActual; j++) {
      const city = faker.helpers.arrayElement(lowRiskCities);
      const firstName = faker.person.firstName();
      const lastName = faker.person.lastName();
      const email = faker.internet.email({ firstName, lastName });
      ...
      const person = {
        ...
        latitude: city.lat + (faker.number.float({ min: -0.1, max: 0.1 })),
        longitude: city.lng + (faker.number.float({ min: -0.1, max: 0.1 })),
        houseValue: generateHouseValue(city, 'low'),
        hasInsurance: faker.datatype.boolean({ probability: 0.5 }), // 50% have insurance
        createdAt: Date.now(),
        updatedAt: Date.now()
      };
    ...
    console.log(`✅ Processed ${processed}/${lowRiskCount} low-risk customers`);
  }
  
  // Generate Bangladesh customers (10% of total)
  const bangladeshCount = Math.floor(count * 0.1);
  console.log(`🇧🇩 Generating ${bangladeshCount} Bangladesh customers...`);
  ...
  for (let i = 0; i < bangladeshCount; i += batchSize) {
    ...
    for (let j = 0; j < batchSizeActual; j++) {
      ...
      const person = {
        ...
        lastName,
        email,
        phone: generateBangladeshPhoneNumber(),
        city: city.name,
        state: 'Bangladesh',
        latitude: city.lat + (faker.number.float({ min: -0.1, max: 0.1 })),
        longitude: city.lng + (faker.number.float({ min: -0.1, max: 0.1 })),
        houseValue: generateBangladeshHouseValue(),
        hasInsurance: faker.datatype.boolean({ probability: 0.2 }), // 20% have insurance
        createdAt: Date.now(),
        updatedAt: Date.now()
      };
    ...
    console.log(`✅ Processed ${processed}/${bangladeshCount} Bangladesh customers`);
  }
  
  const endTime = Date.now();
  const duration = (endTime - startTime) / 1000;
  
  console.log(`🎉 Successfully generated ${processed} people in ${duration.toFixed(2)} seconds!`);
  
  // Get final statistics
  try {
    const totalPeople = await convex.query(api.persons.count);
    const insuredPeople = await convex.query(api.persons.countInsured);
    
    console.log('\n📊 Final Statistics:');
    console.log(`Total people: ${totalPeople.toLocaleString()}`);
    console.log(`Insured people: ${insuredPeople.toLocaleString()} (${(insuredPeople/totalPeople*100).toFixed(1)}%)`);
    
    // Get sample data to verify
    const sampleData = await convex.query(api.persons.getSample, { limit: 5 });
    
    console.log('\n👥 Sample of newly inserted people:');
    sampleData.forEach((person: any, index: number) => {
      ...
      console.log(`   Has Insurance: ${person.hasInsurance ? 'Yes' : 'No'}`);
      console.log('');
    });
...
console.log(`🚀 Starting people generation with count: ${count}`);

generatePeopleConvex(count)
  .catch((e) => {
    console.error('❌ Error generating people:', e);
    process.exit(1);
  });
